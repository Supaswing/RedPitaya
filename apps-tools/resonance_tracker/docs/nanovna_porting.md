...
| `rt_find_resonances`, `rt_find_extrema_candidates`, `rt_select_candidates` | Inflection pairing, extrema fallback, overlap-aware ranking | No | `BaselineAnalyzer::findCandidates` | Faithful bounded-vector port with a Red Pitaya runtime acceptance gate of 50 <= frequency/FWHM <= 150 | Multiple-candidate and out-of-range-Q replay tests | 2A adapted |
