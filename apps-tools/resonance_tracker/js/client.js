(function () {
    ...
    function draw() {
        const canvas = $('plot'), ctx = canvas.getContext('2d'), inc = app.signals.RT_HISTORY_INC_MAG, ref = app.signals.RT_HISTORY_REF_MAG;
    ...
    function render() {
        const state = ['STOPPED','RUNNING','ERROR'][value('RT_STATE',0)]; $('state').textContent=state; $('error').textContent=value('RT_ERROR','No backend error') || 'No backend error'; $('sequence').textContent=value('RT_SEQUENCE',0); $('validity').textContent=value('RT_VALID',false)?'VALID':'INVALID'; $('overflow').textContent=value('RT_OVERFLOW',false)?'OVERFLOW':'OVERFLOW: UNAVAILABLE'; $('effective').textContent=value('RT_EFFECTIVE_FREQUENCY_HZ','-'); $('rate').textContent=Number(value('RT_ACQUISITION_RATE_HZ',0)).toFixed(1); $('connection').textContent=app.ws && app.ws.readyState===1?'CONNECTED':'DISCONNECTED';
        [['inc-i','RT_INC_I'],['inc-q','RT_INC_Q'],['inc-mag','RT_INC_MAG'],['inc-phase','RT_INC_PHASE_DEG'],['ref-i','RT_REF_I'],['ref-q','RT_REF_Q'],['ref-mag','RT_REF_MAG'],['ref-phase','RT_REF_PHASE_DEG']].forEach(function (x) { const v=value(x[1],'-'); $(x[0]).textContent=typeof v==='number'?v.toFixed(x[1].includes('PHASE')||x[1].includes('MAG')?3:0):v; }); draw();
    }
    function showConnection(message, isError) {
        $('connection').textContent = message;
        $('connection').classList.toggle('error', Boolean(isError));
    }
    function retryStart() {
    ...
    function startApp() {
        ...
        $.ajax({url:startUrl, type:'GET', timeout:10000}).done(function (response) {
    ...
    $('run').onclick=function(){ app.running=!app.running; send({RT_RUN:{value:app.running}}); $('run').textContent=app.running?'STOP':'RUN'; $('run').className=app.running?'stop':''; };
    $('frequency').onchange=function(){ send({RT_FREQUENCY_HZ:{value:Number($('frequency').value)}}); };
    $('interval').onchange=function(){ send({RT_TELEMETRY_MS:{value:Number($('interval').value)}}); };
    window.addEventListener('beforeunload', function () {
        ...
        $.ajax({url:stopUrl, type:'GET', async:false});
    });
