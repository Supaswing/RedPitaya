...
if [ "$(id -u)" -ne 0 ]; then
    echo "noise_test.sh must run as root to access /dev/mem" >&2
    exit 2
fi
...
CMAKE_BUILD_PARALLEL_LEVEL=1 cmake --build "$build_dir" --target raw_iq_noise_test
cd "$app_source_dir"
exec "$build_dir/resonance_tracker/raw_iq_noise_test" "$@"
